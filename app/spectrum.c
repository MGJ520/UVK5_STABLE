...
//    if(gScanRangeStop>gScanRangeStart)
//        return (gScanRangeStop - gScanRangeStart) / GetScanStep();
//    else
//        return (gScanRangeStart - gScanRangeStop) / GetScanStep();
//#endif
...
static void ToggleRX(bool on) {
    ...
    // 如果启用了多普勒效应处理
#ifdef ENABLE_DOPPLER
    // 如果处于多普勒模式并且尝试开启接收，则关闭发送
    if (DOPPLER_MODE && on) {
        ToggleTX(false);
    }
#endif

    // 配置自动增益控制，根据是否开启接收和AGC锁定状态来设置
    RADIO_SetupAGC(on, lockAGC);

    // 控制BK4819的GPIO6_PIN2_GREEN引脚，根据是否开启接收来设置状态
    BK4819_ToggleGpioOut(BK4819_GPIO6_PIN2_GREEN, on);

    // 根据是否开启接收来切换音频处理
    ToggleAudio(on);

    // 根据是否开启接收来切换AFDAC（假设是某种音频数字到模拟转换器）
    ToggleAFDAC(on);

    // 根据是否开启接收来切换AFBit（假设是某种位处理）
    ToggleAFBit(on);

    // 如果开启接收
    if (on) {
        // 设置监听时间为1000（单位未知，可能是毫秒）
        listenT = 1000;
        // 根据设置的监听带宽写入相应的寄存器值
        BK4819_WriteRegister(0x43, listenBWRegValues[settings.listenBw]);
    } else {
        // 如果关闭接收，则写入用于扫描的带宽寄存器值
...
static void DrawStatus() {
...
#else
    sprintf(String, "%ddb/%ddb", settings.dbMin, settings.dbMax);
#endif
    ...
    UI_DrawBattery(line + 111, gBatteryDisplayLevel, gLowBatteryBlink);
...
//#endif
 ...
 void RenderFreqInput() {
    UI_PrintStringSmall(freqInputString, 2, 127, 3);
...
//    DrawTicks();
//    绘制刻度线。这通常用于在频谱图上显示频率或振幅的刻度标记，以便用户可以读取和分析频谱数据。
//    DrawArrow(128u * peak.i / GetStepsCount());
//    绘制一个箭头。箭头的位置是基于peak.i（可能表示峰值频率或振幅的索引或位置）和GetStepsCount()（可能返回频谱的步数或刻度数）的值计算出来的。
//    128u可能是一个缩放因子，用于将计算出的位置转换为屏幕上的坐标。
//    箭头通常用于指示频谱中的峰值或特定的重要位置。
//    DrawSpectrum();
//    绘制频谱。这个函数可能负责将频谱数据转换为图形表示，并在屏幕上显示出来。
//    DrawRssiTriggerLevel();
//    绘制RSSI（接收信号强度指示）触发水平线。这通常用于显示一个阈值，当信号强度超过这个阈值时，可能会触发某种动作或通知。
//    DrawF(peak.f);
//    绘制峰值频率。peak.f可能是一个浮点数，表示频谱中的峰值频率，这个函数负责在屏幕上显示这个频率值。
//    DrawNums();
//    绘制数字。这可能指的是在频谱图上绘制刻度标记对应的数值，或者显示其他与频谱相关的数值信息。
    DrawTicks();
    DrawArrow(128u * peak.i / GetStepsCount());
    DrawSpectrum();
    DrawRssiTriggerLevel();
