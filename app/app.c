...
// 将无线电设备从传输模式切换回接收模式
void APP_EndTransmission(bool immediately) {
    // 发送传输结束信号，通知无线电设备传输已完成
    RADIO_SendEndOfTransmission();

    // 如果监视器处于开启状态
    if (gMonitor) {
        // 重新开启监视器，可能是因为传输期间监视器被关闭或更改了配置
        gFlagReconfigureVfos = true;
    }

    // 如果立即切换回接收模式，或者EEPROM中尾音消除功能未启用
    if (immediately || gEeprom.REPEATER_TAIL_TONE_ELIMINATION == 0) {
        // 立即选择前景功能（可能是接收模式）
        FUNCTION_Select(FUNCTION_FOREGROUND);
    } else {
        // 如果不是立即切换，根据EEPROM中配置的尾音消除时间设置倒计时
        // 倒计时单位为10毫秒，因此乘以10来转换为倒计时计数
...
#ifdef ENABLE_FLASHLIGHT
    {   //修改 增加功能 如果发射的时候关闭一下灯,晃眼睛
        if (gCurrentFunction == FUNCTION_TRANSMIT && gFlashLightState) {
            GPIO_ClearBit(&GPIOC->DATA, GPIOC_PIN_FLASHLIGHT);
        } else if (gFlashLightState) {
...
// -------------------- PTT ------------------------
    // 如果 PTT 按钮被按下
    if (gPttIsPressed) {
        // 如果 PTT 引脚被释放或者正在通过串行通信进行配置
        if (GPIO_CheckBit(&GPIOC->DATA, GPIOC_PIN_PTT) || SerialConfigInProgress()) {
            // 如果 PTT 消抖计数器超过 3 次（大约 30 毫秒）或者正在进行串行通信配置
            ...
            // 如果 PTT 引脚没有被释放或者没有串行通信配置正在进行，重置 PTT 消抖计数器
        ...
        // 如果 PTT 引脚没有被按下或者有串行通信配置正在进行，重置 PTT 消抖计数器
        gPttDebounceCounter = 0;
