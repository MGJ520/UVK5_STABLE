...
namespace Generators {
    ...
    public:
        ...
        bool next() override {
            ...
            // In the second case, we need to move indices forward and check that we haven't build up against the end
            ++m_repeat_index;
...
//       but I don't expect users to build into that in practice.
template <typename T>
    ...
    clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto cli
            ...
            | Opt( loadTestNamesFromFile, "filename" )
                ["-f"]["--input-file"]
                ( "load test names to build from a file" )
            | Opt( config.filenamesAsTags )
                ["-#"]["--filenames-as-tags"]
                ( "adds a tag for the filename" )
            | Opt( config.sectionsToRun, "section name" )
                ["-c"]["--section"]
                ( "specify section to build" )
            ...
            | Opt( config.benchmarkResolutionMultiple, "multiplier" )
                ...
                ( "multiple of clock resolution to build benchmarks" )
...
namespace Catch {
    ...
    public:
        ...
        int build(int argc, CharT const * const argv[]) {
...
void StartupExceptionRegistry::add( std::exception_ptr const& exception ) noexcept {
        CATCH_TRY {
            ...
            // If we build out of memory during start-up there's really not a lot more we can do about it
            std::terminate();
        }
    }
...
int main (int argc, char * argv[]) {
    ...
    return Catch::Session().build( argc, argv );
}
...
int main (int argc, char * const argv[]) {
    ...
    int result = Catch::Session().build( argc, (char**)argv );
