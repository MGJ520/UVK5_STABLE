...
//   text,     voice ID,                               menu ID
               ...
               //修改 {/*"Scramb",*/ VOICE_ID_SCRAMBLER_ON, MENU_SCR, 加密}, // was "SCR"
               //修改 {/*"1 Call",*/ VOICE_ID_INVALID, MENU_1_CALL, 按键即呼},
...
//                {/*"F Lock",*/ VOICE_ID_INVALID, MENU_F_LOCK, 频段解锁},
                ...
                {/*"Sql",*/    VOICE_ID_SQUELCH, MENU_BEEP, "BEEP"},
                ...
                {/*"Reset",*/  VOICE_ID_INVALID, MENU_INFO,"SYSTEM"}, // might be better to move this to the hidden menu items ?
...
void UI_DisplayMenu(void) {
...
#endif
    unsigned int i;
    const unsigned int menu_item_x1 = 5;            //最大数量菜单列表上的字符数（左侧）
    const unsigned int menu_item_x2 = LCD_WIDTH - 1;//最大数量菜单列表上的字符数（右侧）
    char String[64];  // 更大的原因是我们现在可以在一个字符串中做多行（使用 'n' char）

    // clear the screen buffer
    UI_DisplayClear();
    //修改位置条--------------------------------------------------------
    if (gIsInSubMenu==false)
    {
        for (uint8_t a = 0; a < 5; a++) {
            PutPixel(125, gMenuCursor+ a, true);
            PutPixel(126, gMenuCursor+ a, true);
            PutPixel(127, gMenuCursor+ a, true);
        }
    }
    ...
    if (gMenuCursor + 1 >= 26)
        add = 0;
    //修改
    sprintf(String, "%2u", add + gMenuCursor);
#else
    //修改
    ...
    if (gIsInSubMenu==false) {
        //显示位置数值
        UI_PrintStringSmall(String, 1, 0, 6);
    }
...
/* 在某些菜单项中，亮度被设置为最大。在我们下次进入这里时，将其恢复到配置的亮度级别。
 * 即，当我们从一 个菜单移动到另一个菜单时。 此外，当按下退出键时，
 * 也需要将其设置回最大亮度。 */
    BACKLIGHT_TurnOn();
...
//-----------------------------------------------------------------------------------------
        case MENU_OFFSET:
        {
            if (!gIsInSubMenu || gInputBoxIndex == 0) {
                ...
                UI_PrintStringSmall(String, menu_item_x1-5, menu_item_x2, 2);
            }
            else
            {
...
//-----------------------------------------------------------------------------------------
        ...
        case MENU_INFO:
        {
            sprintf(String,"FLASH:59/60K \nBAT:%d CYC",cycleCount);
            break;
        }
...
//子菜单序号列表序号显示-----------------------------------------------------------------------------------------
    ...
    {
        if(gIsInSubMenu==true)
        {
            sprintf(String, "%2d", gSubMenuSelection);
            //修改
            UI_PrintStringSmall(String, 107, 0, 6);//small
        }
    }
...
#else
    //显示菜单名称
    UI_PrintStringSmall(MenuList[gMenuCursor].name, size_menu < 48 ? (48 - size_menu) / 2 : 2, 0, 0);
#endif
