            ...
            if (true_char[i] > ' ') {
                ...
                if (index < ARRAY_SIZE(gFontSmall)) {
                    if (flag_move) {
                        ...
                        for (int j = 0; j < 12; ++j) {
                            if (j < 6) gFontSmall_More[j] = (gFontSmall[index][j] & 0x1F) << 3;
                                //00011111
                            else
                                gFontSmall_More[j] = (gFontSmall[index][j - 6] & 0XE0) >> 5;
                            //|(0xFB& *(pFb1+ now_pixel + 1+j-6));//11100000
                        }
...
// x - 横坐标
// y - 纵坐标
// fill - 填充标志，true为填充，false为不填充
void PutPixel(uint8_t x, uint8_t y, bool fill) {
    UI_DrawPixelBuffer(gFrameBuffer, x, y, fill); // 在帧缓冲区绘制像素
}
...
void PutPixelStatus(uint8_t x, uint8_t y, bool fill) {
    UI_DrawPixelBuffer(&gStatusLine, x, y, fill); // 在状态行绘制像素
}
...
void DrawHLine(int sx, int ex, int ny, bool fill) {
    for (int i = sx; i <= ex; i++) { // 从起始横坐标到结束横坐标遍历
        if (i < 128 && ny < 56) { // 确保横坐标在128以内，纵坐标在56以内
            PutPixel(i, ny, fill); // 在指定位置绘制像素
        }
    }
}
...
void DrawVLine(int sy, int ey, int nx, bool fill) {
    for (int i = sy; i <= ey; i++) { // 从起始纵坐标到结束纵坐标遍历
        if (i < 56 && nx < 128) { // 确保纵坐标在56以内，横坐标在128以内
            PutPixel(nx, i, fill); // 在指定位置绘制像素
        }
    }
}
...
void invertScreenLine(uint8_t line) {
    if (line >= 0 && line < (FRAME_LINES * 8)) {
        // 计算在二维数组中的行索引
        uint8_t bufferLine = line / 8;
        // 计算在该行中的位索引
        uint8_t bitIndex = line % 8;

        for (uint8_t col = 0; col < LCD_WIDTH; ++col) {
            // 取反指定行的数据位
            gFrameBuffer[bufferLine][col] ^= (1 << bitIndex);
        }
    }
}

void invertScreen(uint8_t startline,uint8_t endline)
{
