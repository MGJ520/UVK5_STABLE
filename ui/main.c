 ...
 * 绘制小型天线和信号条
 *
 * @param p 指向图形显示缓冲区的指针
 * @param level 信号强度级别，取值范围为0到6，超出范围将被限制为6
 *
 * 该函数首先将天线位图复制到显示缓冲区，然后根据传入的信号级别绘制相应数量的信号条。
 * 信号条的宽度由位操作确定，每个条的高度为2个像素。
 */
static void DrawSmallAntennaAndBars(uint8_t *p, unsigned int level) {
    ...
    for (uint8_t i = 1; i <= level; i++) {
        // 计算信号条的位模式，0xff左移(6 - i)位，然后与0x7F进行与操作
        char bar = (0xff << (6 - i)) & 0x7F;
        // 将信号条绘制到显示缓冲区，位置为p + 2 + i * 3，长度为2个字节
 ...
 * 绘制水平信号条
 *
 * @param xpos 水平位置，信号条开始的x坐标
 * @param line 垂直位置，信号条所在的行
 * @param level 信号条级别，取值范围为0到13，超出范围将被限制为13
 *
 * 该函数根据传入的水平位置、垂直位置和级别绘制水平信号条。
 * 如果定义了ENABLE_AUDIO_BAR_DEFAULT，将使用默认样式绘制信号条；
 * 否则，如果xpos等于35，将绘制特殊样式的信号条。
 * 信号条的每个级别宽度为5个像素，不足9个级别时使用实心样式，
 * 超过9个级别时使用空心样式。
 */
static void DrawLevelBar(uint8_t xpos, uint8_t line, uint8_t level) {
    ...
    // 如果没有定义ENABLE_AUDIO_BAR_DEFAULT，并且xpos等于35，绘制特殊样式的信号条
 ...
 *
 * 该函数负责在主显示屏上显示音频信号条。
 * 如果电池电量低且未确认，或者屏幕正在使用中，
 * 或者设置了闹钟状态，则不显示信号条。
 * 音频信号条的级别基于BK4819音频放大器的输出幅度，
 * 并且通过非线性转换来提高低值时的灵敏度。
 */
void UI_DisplayAudioBar(void) {
    ...
    // 如果当前功能不是传输，或者屏幕不是主显示，或者DTMF调用状态不是无，
    // 则认为屏幕正在使用中，不显示信号条
    ...
    // 如果启用了闹钟或TX1750功能，并且闹钟状态不是关闭，则不显示信号条
    ...
    // 如果启用了中文全功能且未定义英文，且audio_keep_flag为真，
    // 则清除部分行缓冲区，以准备绘制信号条
    ...
    // 根据是否定义了ENABLE_AUDIO_BAR_DEFAULT，在指定位置绘制信号条
#ifndef ENABLE_AUDIO_BAR_DEFAULT
...
//显示频率信息---------------------------------------------------------------------------------------
                switch (gEeprom.CHANNEL_DISPLAY_MODE) {
                    case MDF_FREQUENCY:    // show the channel frequency
...
#endif
                        {
                            // show the frequency in the main font
                            //修改
                            UI_PrintStringSmall(String, 32+6, 1, line);
                        }
                    ...
                    case MDF_CHANNEL:    // show the channel number
                        ...
                        //修改
                        UI_PrintStringSmall(String, 32+6, 0, line);
                        break;
...
#endif
                            //修改
                            UI_PrintStringSmall(String, 32 + 4+2, 0, line);

                            // show the channel frequency below the channel number/name
                            sprintf(String, "%03u.%05u", frequency / 100000, frequency % 100000);
#if ENABLE_CHINESE_FULL != 4 || defined(ENABLE_ENGLISH)
                            //修改
                            UI_PrintStringSmall(String, 32 + 4+2, 0, line + 1);
#else
            ...
            {    // frequency mode
...
#endif
                {
                    // show the frequency in the main font
                    //修改
                    UI_PrintStringSmall(String, 32+6, 0, line+1);
                }
