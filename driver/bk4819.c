...
void BK4819_Init(void) {
    ...
    BK4819_WriteRegister(BK4819_REG_1F, 0x5454);// 0101 0100 01 01 0100
    BK4819_WriteRegister(BK4819_REG_3E, 0xA037);// band selection threshold = VCO max frequency (Hz) / 96 / 640

    BK4819_WriteRegister(BK4819_REG_33, 0x9000);// 1001 0000 0000 0000 .. GPIO
...
void BK4819_PlayRoger(void) {
    ...
    if (gEeprom.ROGER == ROGER_MODE_ROGER || gEeprom.ROGER == ROGER_MODE_MDC_HEAD_ROGER)
        BK4819_PlayRogerNormal(500,700);
#ifdef ENABLE_MDC1200
    else
...
#endif
    } else if (gEeprom.ROGER == ROGER_MODE_OTHER_Motorola)
    {
        BK4819_PlayRogerNormal(1540,1310);
    }
    else if (gEeprom.ROGER == ROGER_MODE_OTHER_Mi)
    {
        BK4819_PlayRogerNormal(1000,1310);
    }
    else if (gEeprom.ROGER == ROGER_MODE_OTHER_HLD)
    {
        BK4819_PlayRogerNormal(1000,850);
    }
#endif
}
...
void BK4819_SetupSquelch(
    ...
    BK4819_WriteRegister(BK4819_REG_4E,  // 01 101 11 1 00 000 000
...
void BK4819_set_AFC(unsigned int level)
{
    ...
    if (level == 0)
        BK4819_WriteRegister(0x73, BK4819_ReadRegister(0x73) | (1u << 4));  // disable
    else
        BK4819_WriteRegister(0x73, ((8u - level) << 11) | (0u << 4));
}
...
void BK4819_SetCompander(const unsigned int mode) {
    ...
    // 2:1压缩比，仅在TX或TX+RX模式下设置
    const uint16_t compress_ratio = (mode == 1 || mode >= 3) ? 2 : 0;
    const uint16_t compress_0dB = 86; // 压缩0 dB点
    const uint16_t compress_noise_dB = 64; // 压缩噪声点
    // 将压缩比、0 dB点和噪声点写入寄存器29
    BK4819_WriteRegister(BK4819_REG_29,
                         (compress_ratio << 14) | // 设置压缩比
                         (compress_0dB << 7) |    // 设置0 dB点
                         (compress_noise_dB << 0)); // 设置噪声点
    ...
    // 1:2扩展比，仅在RX或TX+RX模式下设置
    const uint16_t expand_ratio = (mode >= 2) ? 1 : 0;
    const uint16_t expand_0dB = 86; // 扩展0 dB点
    const uint16_t expand_noise_dB = 56; // 扩展噪声点
    // 将扩展比、0 dB点和噪声点写入寄存器28
    BK4819_WriteRegister(BK4819_REG_28,
                         (expand_ratio << 14) | // 设置扩展比
                         (expand_0dB << 7) |    // 设置0 dB点
                         (expand_noise_dB << 0)); // 设置噪声点
    // 启用
    BK4819_WriteRegister(BK4819_REG_31, r31 | (1u << 3));
}
...
void BK4819_PlayRogerNormal(uint32_t tone1_Hz,uint32_t tone2_Hz) {
